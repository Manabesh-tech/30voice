...
export function AudioPlayer({ voiceNoteId, audioUrl, mp3Url, duration, listenCount }: AudioPlayerProps) {
  ...
  const handlePlayPause = async () => {
    ...
    if (isPlaying) {
      ...
      const incrementListenCount = async () => {
        try {
          // Generate a session ID for tracking unique listens
      ...
      <div className="flex items-center gap-3 p-3 bg-red-50 border border-red-200 rounded-lg">
        ...
        <div className="flex-1">
          <p className="text-red-600 text-sm font-medium">
            {audioState.error?.includes('format not supported') ? 'Format not supported' : 'Audio error'}
          </p>
          <p className="text-red-500 text-xs">
            {audioState.error?.includes('format not supported') 
