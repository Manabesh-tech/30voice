...
interface ReplyFormProps {
  voiceNoteId: string
  parentId?: string // For threaded replies (replies to replies)
...
export function ReplyForm({ voiceNoteId, parentId, onReplySubmitted, onCancel }: ReplyFormProps) {
  const { user } = useAuth()
  ...
  const handleVoiceSubmit = async () => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        // Upload via edge function with threading support
        ...
        if (error) {
          ...
          throw new Error(error.message || 'Failed to upload voice reply')
        }

        console.log('Voice reply uploaded successfully:', data)
        onReplySubmitted()
        setVoiceBlob(null)
        setIsSubmitting(false)
      }
